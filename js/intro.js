...
const INTRO_SCENES = {
    ...
    alistair: {
        ...
        postNameBeats: [
            {
                // Beat 6."A name on the watch-roll" (knight-coded acknowledgment)
                // (Rewrite May 2026 — owner pushback: original "He repeats it
                //  like a vow" with three-fold name repetition didn't fit the
                //  career-captain character. Knights make ONE oath, not three
                //  recitations. Also rendered broken when the player's name was
                //  one letter (e.g. "f. *Quieter.* f. *Slower, almost a vow.*
    ...
    // NOTE: A 35-line `proto: {...}` definition used to live here. It was
    // dead unreachable code \u2014 a second `proto:` key further down (the post-
    // bridge timeline version) silently overrode it via duplicate-key
    // resolution. Removed May 2026 to prevent future edits to the wrong
    // copy. The active Proto intro lives near the bottom of INTRO_SCENES.
